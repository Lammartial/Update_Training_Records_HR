...
"""
HR_IMPORT4 Rev 5.4  (combined A+B)
Author: (generated)
...
def launch_gui():
    ...
    root = tk.Tk()
    root.title("HR_IMPORT Script 2 Rev 5.5 - Training Record Evidence IMPORT")
...
def canonical_key_tuple3(training_no, training_dt, emp_id) -> Tuple[str, str, str]:
    """
    3-field canonical key for duplicate checks: (Training_No., Training_date, Employee_ID)
    Safely converts inputs to exact strings without math truncation.
    """
    ...
    def clean_numlike(x):
        if x is None:
            return ""
        s = str(x).strip()
        # Correctly handles "123.0" float strings from Excel without breaking long barcodes
        if s.endswith(".0"):
            s = s[:-2]
        return s
...
def step1_build_existing_keyset(ws1: Worksheet) -> Tuple[set, int, int]:
    ...
    for r in range(2, max_row + 1):
        ...
        # Prevent ghost keys if the row is entirely blank
...
def step2_dry_run_and_dupe_check(ws1: Worksheet, last_pk: int, last_evid: int) -> Tuple[list, int, int]:
    ...
    for ws_name in wb_src.sheetnames:
        ...
        # Skip standard summary or template sheets naturally
        ...
        # Filter duplicates row-by-row rather than throwing away the entire sheet
